...
impl<'a> Runtime<'a> {
    pub(crate) fn new(module: Module<'a>) -> Self {
        ...
        let machine = target.create_target_machine(&triple, cpu, features, level, reloc_mode, code_model).unwrap();
        module.run_passes("default<O1>", &machine, PassBuilderOptions::create()).unwrap();
        eprintln!("----------------------- After optimize -----------------------");
